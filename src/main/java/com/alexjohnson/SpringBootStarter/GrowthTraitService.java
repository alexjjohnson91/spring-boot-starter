...
public class GrowthTraitService {
    ...
    public List<GrowthPlanTrait> findByGrowthTraitParameters(
            String trait, String status, String mentee_rating, String mentor_rating, String engineer_rating) {
        if (trait == null
                && status == null
                && mentee_rating == null
                && mentor_rating == null
                && engineer_rating == null) {
            return growthTraitRepository.findAll();
        }
        return growthTraitRepository.findByTraitTypeOrStatusOrMenteeRatingOrMentorRatingOrEngineerRating(
                trait, status, mentee_rating, mentor_rating, engineer_rating);
    }

    public GrowthTrait moveTraitToNextStatus(Long id) {
        GrowthTrait growthTrait =
                growthTraitRepository.findById(id).orElseThrow(() -> new GrowthTraitNotFoundException());

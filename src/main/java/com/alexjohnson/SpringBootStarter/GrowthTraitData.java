...
public interface GrowthTraitData {
  ...
  @Value("#{target.result}") String getResult();

  @Value("#{target.opportunity}") String getOpportunity();

  @Value("#{target.nextStep}") String getNextStep();

  @Value("#{target.goal}") String getGoal();

  @Value("#{target.startDate}") Date getStartDate();

  @Value("#{target.status}") Status getStatus();

  @Value("#{target.age}") int getAge();

  @Value("#{target.notes}") String getNotes();
}
